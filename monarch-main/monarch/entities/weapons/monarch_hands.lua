...
local CARRY_POSITION_LAG_SPEED = 7
local CARRY_ANGLE_LAG_SPEED = 9
...
function SWEP:Reset(throw)
	...
	self.CarryHack = nil
	self.Constr = nil
	self.CarryLerpPos = nil
	self.CarryLerpAng = nil
end
...
if SERVER then
	...
	function SWEP:Think()
		...
		local targetPos = self:GetOwner():EyePos() + self:GetOwner():GetAimVector() * (35 + obb)
		local holdPhys = self.HoldingEntity:GetPhysicsObject()
		local holdMass = IsValid(holdPhys) and holdPhys:GetMass() or 0
		local massFactor = math.Clamp((holdMass / CARRY_WEIGHT_LIMIT), 0, 1)
		local posLagSpeed = Lerp(massFactor, CARRY_POSITION_LAG_SPEED, CARRY_POSITION_LAG_SPEED * 0.55)

		self.CarryLerpPos = self.CarryLerpPos or self.CarryHack:GetPos()
		self.CarryLerpPos = LerpVector(math.Clamp(FrameTime() * posLagSpeed, 0, 1), self.CarryLerpPos, targetPos)
		self.CarryHack:SetPos(self.CarryLerpPos)
		...
		self.CarryLerpAng = self.CarryLerpAng or self.CarryHack:GetAngles()
		self.CarryLerpAng = LerpAngle(math.Clamp(FrameTime() * CARRY_ANGLE_LAG_SPEED, 0, 1), self.CarryLerpAng, targetAng)
		self.CarryHack:SetAngles(self.CarryLerpAng)
...
function SWEP:SecondaryAttack()
	...
	if (SERVER and IsValid(entity)) then
		if (entity:IsPlayer() or entity:IsNPC()) then
			local direction = self:GetOwner():GetAimVector() * (300 + 3 * 3)
				direction.z = 0
