...
if CLIENT then
	SWEP.PrintName = "Vehicle Vendor Editor"
	SWEP.Slot = 0
	SWEP.SlotPos = 1
	SWEP.CLMode = 0

	net.Receive("Monarch_VehicleVendor_List", function()
		Monarch = Monarch or {}
		Monarch.VehicleVendorsClient = net.ReadTable() or {}
	end)
end
...
if SERVER then
	function SWEP:Equip(owner)
		if not owner:IsAdmin() then
			owner:StripWeapon("monarch_vehiclevendoreditor")
		end
	end
	...
	function SWEP:PrimaryAttack()
		...
		net.Start("Monarch_VehicleVendor_List")
		net.SendToServer()
		...
		timer.Simple(0.1, function()
			...
			if IsValid(ent) and ent:GetClass() == "monarch_vehiclevendor" then
			local frame = vgui.Create("DFrame")
			frame:SetSize(math.min(ScrW() - 60, 980), math.min(ScrH() - 40, 900))
			frame:Center()
			frame:MakePopup()
			StyleFrame(frame, "Configure Vehicle Vendor")
			...
			local vendors = {}
			local vendorSource = (Monarch and Monarch.VehicleVendorsClient) or (Monarch and Monarch.VehicleVendors)
			...
			for _, v in ipairs(vendors) do
				...
				btn.DoClick = function()
					net.Start("Monarch_SetVehicleVendorID")
					net.WriteEntity(ent)
					net.WriteString(v.id)
					net.SendToServer()
					owner:Notify("Vehicle vendor type set: "..v.name)
				end
			end
			...
			applyBtn.DoClick = function()
				...
				QueueFitModelPreview(preview)
				net.Start("Monarch_SetVehicleVendorModel")
				net.WriteEntity(ent)
				net.WriteString(path)
				net.SendToServer()
				owner:Notify("Vehicle vendor model updated.")
			end
			...
			resetBtn.DoClick = function()
				net.Start("Monarch_ClearVehicleVendorModel")
				net.WriteEntity(ent)
				net.SendToServer()
				preview:SetModel("models/Humans/Group01/male_02.mdl")
				QueueFitModelPreview(preview)
				owner:Notify("Vehicle vendor model cleared to default.")
			end
			...
			removeBtn.DoClick = function()
				net.Start("Monarch_RemoveVehicleVendor")
				net.WriteEntity(ent)
				net.SendToServer()
				frame:Close()
			end
			...
			saveBtn.DoClick = function()
				net.Start("Monarch_SaveSingleVehicleVendor")
				net.WriteEntity(ent)
				net.SendToServer()
			...
			self.NextGo = CurTime() + .3
				...
				owner:Notify("Aim at a monarch_vehiclevendor entity!")
			end
		end)
	end
	...
	function SWEP:SecondaryAttack()
		...
		if IsValid(ent) and ent:GetClass() == "monarch_vehiclevendor" then
			net.Start("Monarch_ResetVehicleVendor")
			net.WriteEntity(ent)
			net.SendToServer()

			self.Owner:Notify("Reset vehicle vendor!")
			surface.PlaySound("buttons/button10.wav")
		else
			self.Owner:Notify("Aim at a monarch_vehiclevendor entity!")
		end
	...
	function SWEP:DrawHUD()
		local line1 = "Vehicle Vendor Editor - Aim at monarch_vehiclevendor"
		...
		for v, k in pairs(ents.FindByClass("monarch_vehiclevendor")) do
...
if SERVER then
	util.AddNetworkString("Monarch_VehicleVendor_List")
	util.AddNetworkString("Monarch_SetVehicleVendorID")
	util.AddNetworkString("Monarch_ResetVehicleVendor")
	util.AddNetworkString("Monarch_SetVehicleVendorModel")
	util.AddNetworkString("Monarch_ClearVehicleVendorModel")
    util.AddNetworkString("Monarch_RemoveVehicleVendor")
    util.AddNetworkString("Monarch_SaveSingleVehicleVendor")

	net.Receive("Monarch_VehicleVendor_List", function(_, ply)
		if not IsValid(ply) or not ply:IsAdmin() then return end
		local payload = {}
		if Monarch and Monarch.VehicleVendors then
			for id, data in pairs(Monarch.VehicleVendors) do
				payload[id] = {
					name = data.name,
					desc = data.desc,
					model = data.model
	...
	net.Receive("Monarch_SetVehicleVendorID", function(len, ply)
		...
		if IsValid(ent) and ent:GetClass() == "monarch_vehiclevendor" then
			...
			local cfg = Monarch and Monarch.VehicleVendors and Monarch.VehicleVendors[vendorID] or nil
			local name = (cfg and cfg.name) or "Vehicle Vendor"
			local desc = (cfg and cfg.desc) or "Purchase and spawn vehicles"
			...
			ply:Notify("Vehicle vendor configured as: " .. vendorID .. " - Click 'Save This Vendor' to save")
		end
	end)

	net.Receive("Monarch_SetVehicleVendorModel", function(_, ply)
		if not ply:IsAdmin() then return end
		local ent = net.ReadEntity()
	...
	net.Receive("Monarch_ClearVehicleVendorModel", function(_, ply)
		if not ply:IsAdmin() then return end
		local ent = net.ReadEntity()
		if IsValid(ent) and ent:GetClass() == "monarch_vehiclevendor" then
	...
	net.Receive("Monarch_ResetVehicleVendor", function(len, ply)
		if not ply:IsAdmin() then return end
		...
		if IsValid(ent) and ent:GetClass() == "monarch_vehiclevendor" then
			ent:SetVendorID("")
			ent:SetVendorName("Vehicle Vendor")
			ent:SetVendorDesc("Purchase and spawn vehicles")
			ent:SetModel("models/Humans/Group01/male_02.mdl")
			ent.MonarchSaveData = ent.MonarchSaveData or {}
			ent.MonarchSaveData.vendorID = ""
			ent.MonarchSaveData.name = "Vehicle Vendor"
			ent.MonarchSaveData.desc = "Purchase and spawn vehicles"
			ent.MonarchSaveData.model = nil
			ply:Notify("Vehicle vendor reset! - Click 'Save This Vendor' to save")
		end
	end)

	net.Receive("Monarch_RemoveVehicleVendor", function(_, ply)
		if not ply:IsAdmin() then return end
		local ent = net.ReadEntity()
		if IsValid(ent) and ent:GetClass()=="monarch_vehiclevendor" then

			if ent._persistUID and Monarch and Monarch._removedVehicleVendorUIDs then
				Monarch._removedVehicleVendorUIDs[ent._persistUID] = true
			end
			ply:Notify("Vehicle vendor removed - Changes will save with other vendors")
			ent:Remove()
		end
	end)

	net.Receive("Monarch_SaveSingleVehicleVendor", function(_, ply)
		if not ply:IsAdmin() then return end
		local ent = net.ReadEntity()
		if IsValid(ent) and ent:GetClass() == "monarch_vehiclevendor" then
			if not Monarch or not Monarch.VehicleVendor_SaveSingle then
				ply:Notify("Error: Monarch VehicleVendor_SaveSingle not initialized")
				return
			end

			local vendorID = ent:GetVendorID()
			if vendorID and vendorID ~= "" then
				ent.MonarchSaveData = ent.MonarchSaveData or {}
				ent.MonarchSaveData.vendorID = vendorID
				ent.MonarchSaveData.name = ent:GetVendorName()
				ent.MonarchSaveData.desc = ent:GetVendorDesc()
				ent.MonarchSaveData.model = ent:GetModel()
				ent.MonarchSaveData.team = ent:GetRequiredTeam()
				Monarch.VehicleVendor_SaveSingle(ent)
				ply:Notify("Vehicle vendor saved successfully")
			else
				ply:Notify("Cannot save: Vendor ID not set")
			end
		end
	end)
end
